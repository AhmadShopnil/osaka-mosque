      ...
      <div className="container mx-auto px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
          ...
          <div className="md:col-span-3">
            <h4 className="text-center text-3xl font-medium mb-3">{prayer_time?.short_description}</h4>
            ...
            <table className="w-full rounded-lg border-collapse hidden md:table">
              ...
              <tbody>
                {localPrayers?.reverse().map((p) => {
                  ...
                  const prayerTimeValue = localRaw ? formatTime(localRaw) : "-"
                  const startRaw = startTimes[prayerName]
                  const startTimeValue = startRaw ? formatTime(startRaw) : "-"
                  const endRaw = getEndTime(prayerName)
                  const endTimeValue = endRaw ? formatTime(endRaw) : "-"
                  return (
                    <tr key={name} className="text-center hover:text-[#00401A] transition">
                      ...
                      <td className="p-3 bg-blue-50">{startTimeValue}</td>
                      <td className="p-3 bg-blue-50">{endTimeValue}</td>
                    </tr>
                  )
                })}
              </tbody>
            </table>
            {/* Mobile Card View - Refactored with raw Tailwind CSS */}
                ...
                return (
                  <div key={name} className="bg-white rounded-lg shadow-md p-4 border border-gray-200">
                    <div className="pb-2 mb-2 border-b border-gray-200">
                      <h3 className="text-lg font-semibold text-[#00401A]">{prayerName}</h3>
                    </div>
                    <div className="grid gap-2 text-sm">
                      <div className="flex justify-between items-center">
                        <span className="font-medium text-gray-600">{prayerTime?.value || "Prayer Time"}:</span>
                        <span className="font-bold text-gray-800">{localRaw}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="font-medium text-gray-600">{startTime?.value || "Start Time"}:</span>
                        <span className="font-bold text-gray-800">{startTimeValue}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="font-medium text-gray-600">{endTime?.value || "End Time"}:</span>
                        <span className="font-bold text-gray-800">{endTimeValue}</span>
                      </div>
                    </div>
                  </div>
                )
