...
// Pillars data
const pillars = [
  { title: 'Shahada', subtitle: '(Faith)', icon: 'faCircleUser', shape: true, flip: false },
  { title: 'Salah', subtitle: '(Prayer)', icon: 'faPersonPraying', shape: true, flip: true },
  { title: 'Zakat', subtitle: '(Charity)', icon: 'faHandHoldingHeart', shape: true, flip: false },
  { title: 'Sawm', subtitle: '(Fasting)', icon: 'faUtensils', shape: true, flip: true },
            ...
            {pillars.map((pillar, index) => (
              <div key={index} className="group flex flex-col items-center gap-4">
                <div className="text-center">
                  <div className="bg-[#00401A] flex items-center justify-center w-[160px] h-[160px]
                   relative border-2 border-white rounded-full ">
                    <FontAwesomeIcon icon={iconMap[pillar.icon]} className="text-white text-[50px]" />
                    {pillar.shape && (
                      <div
                        className={`absolute hidden md:flex -right-24 ${pillar.flip ? 'transform rotate-x-180' : ''}`}
                      >
                        <Image src="/images/pilr-shp.png" alt="Shape" width={100} height={100} />
                      </div>
                    )}
                  </div>
